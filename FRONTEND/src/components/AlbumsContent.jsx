...
function AlbumsContent() {
  ...
  useEffect(() => {
    ...
    let redirectLink = `http://localhost:8080/api/user-top-albums?code=${code}`;
    axios
      .get(redirectLink)
      .then((response) => {
        setAlbums(response.data); // Assuming the response data is an array of tracks
      })
      .catch((error) => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 my-12 sm:my-16">
            {albums
              .filter((album, index, self) => {
                // Use the filter method to keep only the first occurrence of each album
                return (
                ...
                >
                  ...
                  <h1 className="text-base font-extrabold tracking-tight text-gray-900 sm:text-3xl md:text-2xl lg:text-3xl xl:text-2xl">
                    <span className="block xl:inline bg-clip-text text-transparent rounded-b bg-gradient-to-r  from-white to-green-500">
                      <a href={album.album.externalUrls.externalUrls.spotify}>
                        {album.album.name}
                      </a>
                    </span>
                  </h1>
