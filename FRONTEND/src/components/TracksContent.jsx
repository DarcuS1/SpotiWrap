...
function TracksContent() {
  ...
  useEffect(() => {
    ...
    let redirectLink = `http://localhost:8080/api/user-top-tracks?code=${code}`;
    axios
      .get(redirectLink)
      .then((response) => {
        setTracks(response.data); // Assuming the response data is an array of tracks
      })
      .catch((error) => {
              ...
              >
                ...
                <h1 className="text-base font-extrabold tracking-tight text-gray-900 sm:text-3xl md:text-2xl lg:text-3xl xl:text-2xl">
                  <span className="block xl:inline bg-clip-text text-transparent rounded-b bg-gradient-to-r  from-white to-green-500">
                    <a href={track.externalUrls.externalUrls.spotify}>
                      {track.name}
                    </a>
                  </span>
                </h1>
